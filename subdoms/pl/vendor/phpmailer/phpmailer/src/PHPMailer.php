...
class PHPMailer
{
     ...
     * @param string $type        MIME type, e.g. `image/webp`; determined automatically from $path if not specified
     ...
     *                            For example 'file.webp' would get an 'image/webp' MIME type.
     * @param string $encoding    File encoding (see $Encoding), defaults to 'base64'
    ...
    public function msgHTML($message, $basedir = '', $advanced = false)
    {
        ...
        if (array_key_exists(2, $images)) {
            ...
            foreach ($images[2] as $imgindex => $url) {
                ...
                $match = [];
                if (preg_match('#^data:(image/(?:jpe?g|gif|webp));?(base64)?,(.+)#', $url, $match)) {
